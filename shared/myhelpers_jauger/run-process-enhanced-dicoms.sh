...
echo "Processing directory of enhanced DICOMs for volume-to-volume registration."
...
# Docker run command to parse volumes from 4D nifti
...
output_dir="./$output_filename_base-4_nifti_volumes"
mkdir -p "$output_dir"
mv *.nii "$output_dir"
echo "Completed processing of enhanced DICOM files. ITK-compatible volume files saved as: $(basename "$output_filepath")"
