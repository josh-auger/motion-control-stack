...
class Connection:
    ...
    def read_config_text(self):
        ...
        config = config_bytes.decode("utf-8").split('\x00', 1)[0]  # Strip off null teminator
    ...
    def send_image(self, images):
        with self.lock:
            ...
            for image in images:
                ...
                logging.info("--> Sent image %d", self.sentImages)
                time.sleep(self.delay)  # add pause between each image send to simulate scanner acquisition delay
    ...
    def read_image(self):
        ...
        image = ismrmrd.Image(header_bytes, attribute_bytes.decode('utf-8').split('\x00', 1)[0])  # Strip off null terminator

        logging.info("\tImage is size %d x %d x %d with %d channels of type %s", image.matrix_size[0],
                     image.matrix_size[1], image.matrix_size[2], image.channels,
    ...
    def read_feedback(self):
        ...
        # logging.info("data.myBool  is: %s" % dataStruct.myBool)
        # logging.info("data.myInt64 is: %s" % dataStruct.myInt64)
        # logging.info("data.myFloat is: %s" % dataStruct.myFloat)
