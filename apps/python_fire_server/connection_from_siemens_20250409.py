...
class Connection:
    ...
    def send_text(self, contents):
        with self.lock:
            # DNF logging.info("--> Sending MRD_MESSAGE_TEXT (5)")
            # DNF logging.info("    %s", contents)
    ...
    def read_image(self):
        self.recvImages += 1
        # DNF logging.info("<-- Received MRD_MESSAGE_ISMRMRD_IMAGE (1022)")
        ...
        # DNF logging.info("    Image is size %d x %d x %d with %d channels of type %s", image.matrix_size[0], image.matrix_size[1], image.matrix_size[2], image.channels, ismrmrd.get_dtype_from_data_type(image.data_type))
